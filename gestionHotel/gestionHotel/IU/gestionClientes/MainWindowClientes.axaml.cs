...
namespace gestionHotel.IU.gestionClientes
{
    public class MainWindowClientes : Window
    {
        ...
        async void OnInsert()
        {
            ...
            {
                if (comprobarCampos(insertView.Dni, insertView.Nombre, insertView.Telefono,
                    insertView.Email, insertView.Direccion))
                {
                    ...
                    {
                        var confirmar =
                            new GeneralMessage("Ya existe un cliente con este Dni, desea modificalo?", true);
        ...
        async void OnMod(Cliente cliente)
        {
            ...
            else
            {
                ...
                {
                    if (comprobarCampos(cliente.Dni, modifyView.Nombre, modifyView.Telefono,
        ...
        async void OnRes(Cliente cliente)
            ...
            async void OnGraphClienteInd()
            {
                await new GrafReservasIndividuales().ShowDialog(this);
            }
            
            public bool comprobarCampos(string dni, string nombre, int telefono, string email, string direccion)
            {
                if (dni != "" && nombre != "" && email != "" && direccion != "")
                {
                    if (telefono == -1)
                    {
                        new GeneralMessage("El campo Teléfono debe se numérico", false).Show();
                ...
                else
                {
                    ...
                    new GeneralMessage("Hay campos vacíos", false).Show(); 
